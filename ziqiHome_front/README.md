...
- 公开站点：主页、最新动态、博客列表/详情、项目总览、联系平台页
- 首页只请求后台标记为精选的 Moments，并以最多 `3` 条图文和 `2` 条文字提供内容入口
- 瞬间页使用图文与纯文字混排的双列帖子流，并支持服务端分类筛选
- 博客列表使用紧凑编辑式布局，支持服务端关键词、分类和多标签交集筛选，并对低质量摘要、重复默认封面和封面加载失败提供展示降级
- 项目总览使用紧凑 `3 + 2` 网格，并通过独立筛选项接口完成服务端阶段筛选
- 联系页使用三列紧凑平台索引，整卡支持键盘与外部链接访问
- 后台管理：博客、项目、动态 moments、联系平台、用户管理
  其中博客管理已拆分为“正文编辑页 + 文章信息设置”，正文编辑支持 Markdown 实时预览；摘要设置提供质量提示，语雀同步会清洗无效描述
- 登录鉴权：`/admin/login` 登录，`/admin/*` 通过路由守卫保护
- 接口联动：公开页通过 `/api/site/*` 获取数据，管理页通过 `/api/admin/*` 做 CRUD
- 主题系统：默认暗色，支持亮色切换
- 响应式布局：公开站点和管理端都按移动端可用性设计，`860px` 以下公共导航使用紧凑下拉菜单
